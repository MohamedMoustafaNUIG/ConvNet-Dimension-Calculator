...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        #drop down list to choose type of layer
        ...
        #input for number of kernels; for convolutional layers
        ...
        #input to specify padding (if any); for convolutional layer
        ...
        #text boxes to input input dimensions
        ...
        #labels used to display calculated output
        ...
        #label to display error message if calculation fails
        self.error_label = QtWidgets.QLabel(self.centralwidget)
        self.error_label.setGeometry(QtCore.QRect(450, 220, 101, 30))
    ...
    def calculate(self):
        _translate = QtCore.QCoreApplication.translate
        try:
            h = int(self.inp_height_txt.toPlainText())
            w = int(self.inp_width_txt.toPlainText())
            d = int(self.inp_depth_txt.toPlainText())
            
            k_s = int(self.kernel_size_txt.toPlainText())
            s = int(self.stride_txt.toPlainText())
            
            if self.comboBox.currentIndex() == 0:#if conv
                k_n = int(self.kernel_num_txt.toPlainText())
                p = int(self.padding_txt.toPlainText())
                new_h, new_w, new_d = self.conv_layer(h, w, d, k_n, k_s, p, s)
            else:#if pool
                new_h, new_w, new_d = self.pooling_layer(h, w, d, k_s, s)
            
            self.out_height_label.setText(_translate("MainWindow", str(new_h)))
            self.out_width_label.setText(_translate("MainWindow", str(new_w)))
            self.out_depth_label.setText(_translate("MainWindow", str(new_d)))
        except:#if error
            self.error_label.setText(_translate("MainWindow", "Error! Fill all text boxes with integers!"))
            self.update()
    ...
    def update(self):#adjust sizes
        self.out_width_label.adjustSize()
        self.out_height_label.adjustSize()
        self.out_depth_label.adjustSize()
        self.error_label.adjustSize()

    def on_layer_changed(self, value):#disable/enable certain inputs depending on chosen layer
        if value == 0:
            self.padding_txt.setDisabled(False)
            self.kernel_num_txt.setDisabled(False)
        else:
            self.padding_txt.setDisabled(True)
            self.kernel_num_txt.setDisabled(True)

    def conv_layer(self, in_h, in_w, in_d, kernels, kernel_size, padding, stride):#calculate output dimension of convolutional layer
        new_h = int((in_h-kernel_size+(2*padding))/stride)+1
        new_w = int((in_w-kernel_size+(2*padding))/stride)+1
        new_d = kernels
        return new_h, new_w, new_d

    def pooling_layer(self, in_h, in_w, in_d, window_size, stride):#calculate output dimension of pooling layer
